		...
		<div class="row marginB20" style="margin-top: -10px;">
			...
			<div class="col-xs-3 col-sm-3">
				<div id="" class="form-group form-group-sm">
					...
					<input style="" type="file" name="audio_upload" id="audio_upload" accept="audio/*" OnClick="isEditSound = 1;alert(isEditSound);"/>
				</div>
			</div>
			<div class="col-xs-3 col-sm-3">
				<div id="" class="form-group form-group-sm">
...
var isEditSound = 0;
	$(document).ready(function(){
		...
		$("#createform2").on("submit",function(e){
			...
			formData2.append("isEditPic",isEditPic);
			formData2.append("isEditSound",isEditSound);
			//-------------------------
			...
			if (typeof str === "undefined" || $("#description_page").val() == ""  ) {
				...
				///////อัพเดทลงฐานข้อมูล
				$.ajax({
		            url: 'qs/qs_user_updateStory_page.php',
		            type: 'POST',
				    data: formData2,
		            async: false,
		            cache: false,
		...
		        }).done(function(data){ alert("data= "+data);
					if(data == "notsuccess"){
						alert("อัพโหลดไฟล์ไม่ได้ กรุณาลองใหม่");
					}
					else if(data == "failed"){
